...
__global__ void gemm_kernel_block(DATA_TYPE *a, DATA_TYPE *b, DATA_TYPE *c)
{
...
void gemmCuda(DATA_TYPE* A, DATA_TYPE* B, DATA_TYPE* C, DATA_TYPE* C_outputFromGpu)
{
	...
	cudaMemPrefetchAsync(A, sizeof(DATA_TYPE) * NI * NK, 0,0);
	cudaMemPrefetchAsync(B, sizeof(DATA_TYPE) * NK * NJ, 0,0);
	cudaMemPrefetchAsync(C, sizeof(DATA_TYPE) * NI * NJ, 0,0);
	...
	gemm_kernel_block<<< grid, block >>>(A, B, C);
...
int main(int argc, char *argv[])
{
	...
	cudaMallocManaged((void **)&A, sizeof(DATA_TYPE) * NI * NK);
	cudaMallocManaged((void **)&B, sizeof(DATA_TYPE) * NK * NJ);
	cudaMallocManaged((void **)&C, sizeof(DATA_TYPE) * NI * NJ);
	cudaMallocManaged((void **)&C_outputFromGpu, sizeof(DATA_TYPE) * NI * NJ);
