...
void fdtdCuda(DATA_TYPE* _fict_, DATA_TYPE* ex, DATA_TYPE* ey, DATA_TYPE* hz, DATA_TYPE* hz_outputFromGpu)
{
	...
	cudaMemcpyAsync(ex_gpu+i_base*(NY+1), ex+i_base*(NY+1), sizeof(DATA_TYPE) * CHUNK_SIZE * (NY + 1), cudaMemcpyHostToDevice,streams[t % NUM_STREAMS]);
	cudaMemcpyAsync(ey_gpu+i_base*NY, ey+i_base*NY, sizeof(DATA_TYPE) * CHUNK_SIZE * NY, cudaMemcpyHostToDevice,streams[t % NUM_STREAMS]);
	cudaMemcpyAsync(hz_gpu+i_base*NY, hz+i_base*NY, sizeof(DATA_TYPE) * CHUNK_SIZE * NY, cudaMemcpyHostToDevice,streams[t % NUM_STREAMS]);
	

	fdtd_step1_kernel<<<grid,block,0,streams[t % NUM_STREAMS]>>>(_fict_gpu, ex_gpu, ey_gpu, hz_gpu, i_base);
	fdtd_step2_kernel<<<grid,block,0,streams[t % NUM_STREAMS]>>>(ex_gpu, ey_gpu, hz_gpu, i_base);
	
	if (t>0){
	fdtd_step3_kernel<<<grid,block,0,streams[t % NUM_STREAMS]>>>(ex_gpu, ey_gpu, hz_gpu, i_base-CHUNK_SIZE,hz_gpu_out);

	cudaMemcpyAsync(hz_outputFromGpu+(i_base-CHUNK_SIZE)*NY, hz_gpu_out+(i_base-CHUNK_SIZE)*NY, sizeof(DATA_TYPE) * CHUNK_SIZE * NY, cudaMemcpyDeviceToHost,streams[t % NUM_STREAMS]);	
	
	}else{
	
	cudaMemcpyAsync(hz_outputFromGpu+(i_base-CHUNK_SIZE)*NY, hz_gpu_out+(i_base-CHUNK_SIZE)*NY, 0, cudaMemcpyDeviceToHost,streams[t % NUM_STREAMS]);	
	}
	...
	fdtd_step3_kernel<<<grid,block,0,streams[(NUM_CHUNK-1) % NUM_STREAMS]>>>(ex_gpu, ey_gpu, hz_gpu, NX-CHUNK_SIZE,hz_gpu_out);
	cudaMemcpyAsync(hz_outputFromGpu+(NX-CHUNK_SIZE)*NY, hz_gpu_out+(NX-CHUNK_SIZE)*NY, sizeof(DATA_TYPE) * CHUNK_SIZE * NY, cudaMemcpyDeviceToHost,streams[(NUM_CHUNK-1) % NUM_STREAMS]);	
