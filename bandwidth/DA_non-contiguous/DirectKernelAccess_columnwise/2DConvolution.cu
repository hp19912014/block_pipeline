...
__global__ void Simplekernel2d_pre(DATA_TYPE *dst, DATA_TYPE *src)
{
   
  int base,offset;
  const int row_id = (threadIdx.x >> 5); 
  const int row_offset = (threadIdx.x & 31);
  
  for (int i = 0; i < 32 ; i++) {
	for (int j = 0; j < 16; j++){  
	base = 131072*j+32768*blockIdx.x+32*i;    //1024*32*4 = 131072  1024*32=32768
	offset = base + 1024* row_id + row_offset;
	reinterpret_cast<double2*>(dst)[offset] = reinterpret_cast<double2*>(src)[offset];
	}
  }
}
__global__ void Simplekernel2d(DATA_TYPE *dst, DATA_TYPE *src)
{
  
  const int row_id = (threadIdx.x / 32);
  const int row_offset = (threadIdx.x % 32); 

  for (int bid = blockIdx.x ; bid < 512; bid += NUM_SM){
  int block_row = bid % 32;
  int block_offset = bid / 32;
  int offset = 16384*block_row+512*row_id+block_offset*32+row_offset;
  reinterpret_cast<double2*>(dst)[offset] = reinterpret_cast<double2*>(src)[offset];
...
void bandwidthtest(DATA_TYPE* A,DATA_TYPE* B)
{
	...
	Simplekernel2d<<<grid,block,0,0>>>(B_gpu, A);
	...
	//simplekernel DtoH
	cudaEventRecord(start[1]);
	Simplekernel2d<<<grid,block,0,0>>>(B, A_gpu);
	cudaEventRecord(stop[1]);
	cudaDeviceSynchronize();
	cudaEventSynchronize(stop[1]);
	cudaEventElapsedTime(&elapsedTimeInMs, start[1], stop[1]);	
